...
class SalesMixin:
    ...
    @staticmethod
    def _resolve_customer_address(si_doc, source_doc=None):
        """BUG 24 FIX: Resolve customer_address on a Sales Invoice.
        
        ERPNext's make_sales_invoice() does NOT always copy customer_address,
        especially when the Customer has no default billing address set.
        This leaves the SI with customer_address=None, which causes CFDI
        stamping to fail (SAT requires the fiscal address).
        
        Resolution order:
        1. Already set on SI — skip
        2. Copy from source_doc (SO or DN) customer_address field
        3. If source_doc is DN, trace to SO and copy from there
        4. Look up Customer default billing address via Dynamic Link
        5. Look up ANY address linked to Customer (first found)
        
        Also sets company_address from source_doc if missing on SI.
        """
        # Skip if already resolved
        if getattr(si_doc, 'customer_address', None):
            return
        
        customer = getattr(si_doc, 'customer', None)
        
        # --- Strategy 1: Copy from source_doc ---
        if source_doc:
            src_addr = getattr(source_doc, 'customer_address', None)
            if src_addr:
                si_doc.customer_address = src_addr
                # Also copy shipping_address_name if available
                if not getattr(si_doc, 'shipping_address_name', None):
                    ship_addr = getattr(source_doc, 'shipping_address_name', None)
                    if ship_addr:
                        si_doc.shipping_address_name = ship_addr
                # Also copy company_address if missing
                if not getattr(si_doc, 'company_address', None):
                    comp_addr = getattr(source_doc, 'company_address', None)
                    if comp_addr:
                        si_doc.company_address = comp_addr
                return
            
            # --- Strategy 2: If source is DN, trace DN→SO ---
            src_doctype = getattr(source_doc, 'doctype', '')
            if src_doctype == 'Delivery Note':
                try:
                    for item in (getattr(source_doc, 'items', []) or []):
                        so_name = getattr(item, 'against_sales_order', None)
                        if so_name:
                            so_addr = frappe.db.get_value(
                                'Sales Order', so_name, 'customer_address'
                            )
                            if so_addr:
                                si_doc.customer_address = so_addr
                                # Also grab company_address from SO
                                if not getattr(si_doc, 'company_address', None):
                                    so_comp_addr = frappe.db.get_value(
                                        'Sales Order', so_name, 'company_address'
                                    )
                                    if so_comp_addr:
                                        si_doc.company_address = so_comp_addr
                                return
                except Exception:
                    pass
        
        # --- Strategy 3: Customer default billing address ---
        if customer:
            try:
                # ERPNext stores addresses via Dynamic Link
                # Look for address with address_type='Billing' first
                ...
                if billing_addr:
                    si_doc.customer_address = billing_addr[0].name
                    return
                
                # --- Strategy 4: ANY address linked to customer ---
                ...
                """, customer, as_dict=True)
                if any_addr:
                    si_doc.customer_address = any_addr[0].name
                    return
            except Exception:
                pass
    ...
    def _handle_sales_commands(self, query: str, query_lower: str, is_confirm: bool = False) -> Optional[Dict]:
        ...
        if so_match and ("create delivery" in query_lower or "ship" in query_lower or "deliver" in query_lower or "nota de entrega" in query_lower or "!delivery" in query_lower or "delivery note" in query_lower):
            try:
                ...
                # --- BUG 23 FIX: Idempotency guard for Delivery Note ---
                # Check if a DN already exists for this SO (non-cancelled).
                # Prevents duplicate DNs on retry/re-run.
                from raven_ai_agent.api.truth_hierarchy import check_existing_dn
                existing_dn = check_existing_dn(so_name)
                if existing_dn:
                    site_name = frappe.local.site
                    dn_link = f"[{existing_dn}](https://{site_name}/app/delivery-note/{existing_dn})"
                    return {
                        "success": True,
                        "message": f"⚠️ Delivery Note already exists for {so_name}: {dn_link}\n\nSkipping duplicate creation (idempotency guard BUG23)."
                    }
        ...
        if (so_match or dn_match) and ("invoice" in query_lower or "factura" in query_lower):
            try:
                # --- BUG 23 FIX: Idempotency guard for Sales Invoice ---
                # Check if SI already exists for the SO (non-cancelled).
                ...
                if so_match:
                    ...
                    try:
                        ...
                        for _item in (_dn_tmp.items or []):
                            if getattr(_item, 'against_sales_order', None):
                                _idem_so = _item.against_sales_order
                                break
                    except Exception:
                        pass
                if _idem_so:
                ...
                if dn_match:
                    ...
                    # --- BUG 24 FIX: Resolve customer_address ---
                    # Copy customer_address from linked SO, or fall back to
                    # Customer master default billing address.
                    self._resolve_customer_address(si, dn)
                    ...
                    addr_msg = f"\n  📍 Address: {si.customer_address}" if getattr(si, 'customer_address', None) else ""
                    return {
                        "success": True,
                        "message": f"✅ Sales Invoice created: [{si.name}](https://{site_name}/app/sales-invoice/{si.name})\n  Customer: {si.customer}\n  Currency: {si.currency}\n  Total: {si.currency} {si.grand_total:,.2f}{date_msg}\n  🇲🇽 CFDI: {si.mx_payment_option} | Use: {si.mx_cfdi_use} | Pay: {si.mode_of_payment}{fx_msg}{addr_msg}"
                    }
                elif so_match:
                    so_name = so_match.group(1)
                    so = frappe.get_doc("Sales Order", so_name)
                    ...
                    if custom_posting_date:
                        ...
                        si.posting_date = custom_posting_date
                    # --- BUG 24 FIX: Resolve customer_address ---
                    # Copy customer_address from SO, or fall back to
                    # Customer master default billing address.
