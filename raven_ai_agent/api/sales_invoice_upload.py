# -*- coding: utf-8 -*-
"""
Phase 10.2.2: Upload Pedimento for Sales Invoice
Handles file upload to Drive and links to Sales Invoice
"""
...
@frappe.whitelist()
def upload_pedimento(sales_invoice_name: str, file_url: str = None, file_content: str = None):
    """
    Upload Pedimento PDF for a Sales Invoice to Drive.
    ...
    # Get mapping for Sales Invoice Pedimento
    mapping = get_mapping_for_doctype("Sales Invoice", "Pedimento")
    if not mapping:
        frappe.throw(_("No active Drive Mapping Rule found for Sales Invoice Pedimento"))
    
    # Resolve folder and filename
    folder_path = resolve_folder_path(mapping, sales_invoice)
    filename = resolve_filename(mapping, sales_invoice, "pdf")
    ...
    # Update Sales Invoice link field
    sales_invoice.pedimento_file = drive_file.name
    ...
    return {
        ...
        "message": _("Pedimento uploaded successfully to {0}").format(folder_path)
    }
